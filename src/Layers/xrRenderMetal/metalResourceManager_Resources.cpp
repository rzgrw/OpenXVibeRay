// metalResourceManager_Resources.cpp — Metal-specific resource creation
// dispatch. Mirrors glResourceManager_Resources.cpp; shader objects use
// uint64_t opaque handles (see SH_Atomic.h USE_METAL branches).
...
SDeclaration* CResourceManager::_CreateDecl(const D3DVERTEXELEMENT9* dcl)
{
    ...
    D->dcl = 0; // filled by ConvertVertexDeclaration (MTL::VertexDescriptor* handle)
...
bool CResourceManager::_LinkPP(SPass& pass)
{
    ...
    // TODO Task 16: Metal render pipeline states are built lazily at draw
    // time from (vs, ps, vertex descriptor, attachment formats, blend state)
    // and cached — there is no ahead-of-time link step like GL program
    // pipelines. Keep the SPP as a named placeholder for now.
    return true;
}
