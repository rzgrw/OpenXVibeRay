// metalDetailManager_VS.cpp — detail-object (grass) rendering for the Metal backend.
// Mirrors glDetailManager_VS.cpp: instances are packed into the "array" shader
// constant (4 x Fvector4 per instance) and flushed in hw_BatchSize batches.
#include "stdafx.h"
    ...
    for (u32 O = 0; O < objects.size(); O++)
    {
        ...
        if (!vis.empty())
        {
            for (u32 iPass = 0; iPass < Object.shader->E[lod_id]->passes.size(); ++iPass)
            {
                ...
                //	This could be cached in the corresponding constant buffer
