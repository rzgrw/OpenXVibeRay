// r2_R_sun.cpp — old-style (non-cascaded) sun shadow-map rendering for the
// Metal backend: render_sun_old.  Byte-for-byte the GL implementation
// (xrRenderPC_GL/r2_R_sun.cpp): all math is glm-based (r2_R_sun_support.h) and
// all drawing goes through the CBackend abstraction on the immediate command
// list (RCache), i.e. encoder usage stays on the main thread.  The cascaded
// path (render_sun) lives in the shared xrRender_R2/render_phase_sun.cpp.
#include "stdafx.h"
