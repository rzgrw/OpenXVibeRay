// metal_rendertarget_phase_combine.cpp — final combine, volumetric combine and
// wallmark setup for the Metal backend.  Mirrors
// gl_rendertarget_phase_combine.cpp; the only Metal-specific difference is the
// exposure-pipeline LUM texture binding: Metal CTexture::surface_set takes a
// bare uint64_t MTL::Texture* handle (no GL target enum).
// rt_Generic_*_r->resolve_into() in the o.msaa path is a Metal stub (Task 20);
// the path is unreachable until the backend reports MSAA support.
#include "stdafx.h"
...
void CRenderTarget::phase_combine()
{
    ...
    {
        t_LUM_src->surface_set(rt_LUM_pool[gpu_id * 2 + 0]->pRT);
        t_LUM_dest->surface_set(rt_LUM_pool[gpu_id * 2 + 1]->pRT);
    }
