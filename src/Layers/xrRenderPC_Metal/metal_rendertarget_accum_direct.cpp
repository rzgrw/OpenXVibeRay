// metal_rendertarget_accum_direct.cpp — direct (sun) light accumulation for
// the Metal backend.  Mirrors gl_rendertarget_accum_direct.cpp: the code is
// written entirely against the CBackend abstraction (set_Element/set_Stencil/
// Render/...), so it matches the GL implementation line-for-line.  Notes:
//  - u_stencil_optimize() is only reachable when RImplementation.o.nvstencil
//    is set (an NVIDIA GL-driver workaround); it stays false on Metal, so the
//    guarded calls below are dead — kept for GL parity.
//  - The MSAA branches (o.msaa / o.msaa_opt) are kept for parity but are
//    unreachable until the Metal backend reports MSAA support (Task 20).
#include "stdafx.h"
